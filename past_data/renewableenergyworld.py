...
from settings import get_request, get_scrape_do_requests, ReNewableEnergyWorld_client as news_details_client, yourstory_scrape_do_requests
from datetime import datetime
...
logger = CustomLogger(log_folder="/home/riken/news-scrapper/news-scrapper/past_data/Logs/renewableenergyworld")
URLS_list = [
...
class ReNewableEnergyWorld:
    ...
    def separate_blog_details(self, response, grid):
        ...
        try:
            ...
            details['author'] = data.find('span',class_ = "meta-author-name").get_text(strip = True) if data.find('span',class_ = "meta-author-name") else ""
            details['time'] = data.find('div',class_ = "post-meta-date").get_text(strip = True) if data.find('div',class_ = "post-meta-date") else ""
            
            content = data.find('div',class_ = "entry-content")
            if content :
                details['image'] = content.find('img').get('src') if content.find('img') else ""
                details['description']['details'] = ' \n'.join([i.get_text(strip=True) for i in content.find_all('p')])
