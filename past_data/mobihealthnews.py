...
from settings import get_request, get_scrape_do_requests, MobileHealthNews_client as news_details_client, yourstory_scrape_do_requests
from datetime import datetime
...
logger = CustomLogger(log_folder="/home/riken/news-scrapper/news-scrapper/past_data/Logs/MobileHealthNews")
URLS_list = [
...
class MobileHealthNews:
        ...
        for children in data.find_all('div',{'class':'content-list-card'}): 
            try:
                ...
                if link_ele :
                    ...
                    tmp['url'] = f"https://www.mobihealthnews.com{link}" if not "https://www.mobihealthnews.com" in link else link
                    
                # image
                image_ele = children.find('img')
                if image_ele :
                    src = image_ele.get('src')
                    tmp['image'] = f"https://www.mobihealthnews.com{src}" if not "https://www.mobihealthnews.com" in src else src
    ...
    def separate_blog_details(self, response, grid):
        ...
        try:
            ...
            if details_ele :
                details['description']['details'] = ' \n'.join([i.get_text(strip=True) for i in details_ele.children if i.name == "p"])
        except Exception as e:
